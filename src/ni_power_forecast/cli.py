...
from ni_power_forecast.backtest import save_backtest_outputs, walk_forward_backtest
...
def generate_demo(
    days: int = typer.Option(300, min=30),
    output: Path = typer.Option(Path("data/demo/ni_power_demo.csv")),
    seed: int = typer.Option(42),
):
    """Generate a reproducible synthetic Northern Ireland-like hourly dataset."""
    path = write_demo_data(output, days=days, seed=seed)
...
def backtest(
    ...
    n_splits: int = typer.Option(5, min=2, max=10),
):
    """Run expanding-window chronological backtests against a 24h persistence baseline."""
    df = load_market_frame(input)
    predictions, metrics = walk_forward_backtest(df, model_name=model, n_splits=n_splits)
    save_backtest_outputs(predictions, metrics, output_dir)
...
def train(
    ...
    """Fit a model to all usable observations and persist it with joblib."""
    df = load_market_frame(input)
    path = train_model(df, model_name=model, output_dir=output_dir)
...
def forecast(
    ...
    """Forecast rows whose price target is empty using a previously trained model."""
    df = load_market_frame(input)
    path = forecast_missing_targets(df, model_dir=model_dir, output_path=output)
...
def normalise_soni(
    ...
    """Normalise a downloaded SONI System Data Excel workbook to canonical columns."""
    path = normalise_soni_excel(input, output, mapping_file=mapping)
...
def fetch_semo_raw(
    ...
    """Download raw public SEMOpx day-ahead report documents through the SEMO report API."""
    paths = download_report_documents(date_from, date_to, output_dir)
...
def fetch_semo_prices(
    ...
    frame = fetch_day_ahead_prices(date_from, date_to, market=market, currency=currency)
    output.parent.mkdir(parents=True, exist_ok=True)
    frame.to_csv(output, index=False)
...
def parse_semo_prices(
    ...
    """Parse a downloaded EA-001 JSON document into timestamp/price rows."""
    frame = parse_market_result_file(input, market=market, currency=currency)
    output.parent.mkdir(parents=True, exist_ok=True)
    frame.to_csv(output, index=False)
    typer.echo(f"Wrote {len(frame)} price observations to {output}")
...
def inspect_semo(
    ...
    """Flatten a raw SEMO document into a searchable table for schema inspection."""
